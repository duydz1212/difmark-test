  ...
  return (
    <div className="w-[252px] overflow-hidden rounded-lg cursor-pointer transition-all duration-300 hover:brightness-125 hover:shadow-lg hover:shadow-white/10">
      <div className="w-full h-[90px] bg-[#212937] px-4 flex items-center">
        ...
        <div className="pl-3">
          ...
          <div className="flex items-center gap-x-2">
            ...
            {[...Array(5)].map((_, i) => (
              <ImStarFull key={i} className="text-yellow-300 text-xs" />
            ))}
          </div>
        </div>
      </div>
      <div className="w-full h-[44px] bg-[#2b3545] flex items-center justify-between px-4">
        ...
        <div className="flex text-[#C0C3C9] items-center gap-x-2 hover:text-white transition-colors">
    ...
    <div className="w-full my-20 rounded-lg">
      ...
      <div className="relative ">
        ...
        <Button
          variant="outline"
          size="icon"
          className={`absolute rounded-full -left-16 top-1/2 flex items-center -translate-y-1/2 bg-slate-700/90 border-slate-600 text-white hover:bg-slate-600 hover:scale-110 transition-all duration-200 shadow-lg backdrop-blur-sm z-20 h-10 w-10 ${
            !canScrollPrev ? "opacity-50 cursor-not-allowed" : ""
          }`}
          onClick={scrollPrev}
          disabled={!canScrollPrev}
        >
          <MdArrowBackIos className="translate-x-1 h-4 w-4" />
        </Button>

        <Button
          variant="outline"
          size="icon"
          className={`absolute -right-16 top-1/2 rounded-full flex items-center -translate-y-1/2 bg-slate-700/90 border-slate-600 text-white hover:bg-slate-600 hover:scale-110 transition-all duration-200 shadow-lg backdrop-blur-sm z-20 h-10 w-10 ${
