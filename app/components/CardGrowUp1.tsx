...
const GameCard: React.FC<GameCardProps> = ({ cardId }) => {
  ...
  if (!data) {
    return (
      <div className="w-[555px] h-[300px] bg-gray-800 rounded-md flex items-center justify-between">
        ...
        <div className="w-full h-full bg-[#131B25] cursor-pointer transition-all duration-300">
          ...
          <div className="text-white h-full w-full flex flex-col justify-between p-6 relative z-10">
            <div>
              ...
              <span className="text-gray-400 w-full hidden 1000:block max-w-[270px]">
                {data.description}
              </span>
            </div>
...
export default function CardGrowUp1() {
  const displayCards = [
    [1, 2], // Hàng đầu tiên
  ];
  ...
  return (
    <div className="flex flex-col gap-y-10">
      {displayCards.map((row, rowIndex) => (
        <div key={rowIndex} className="flex justify-between gap-x-8 w-full">
          {row.map((cardId) => (
