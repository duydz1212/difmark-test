...
const DynamicBackgroundCarousel: React.FC = () => {
  ...
  return (
    <div className="relative w-full h-[700px] overflow-hidden">
      ...
      {/* Main Content */}
      <div className="relative z-10 flex flex-col justify-center items-start h-full max-w-7xl mx-auto responsive">
        <div
          className={`transition-all relative duration-700 ${
              ...
              <div className="flex gap-1">
                {backgrounds.map((_, index) => (
                  <div key={index} className="flex-1 relative group">
                    <div
                      className="h-[2px] bg-white/20 rounded-full cursor-pointer overflow-hidden"
                      onClick={() => changeSlide(index)}
                    >
                      {/* Progress bar - chỉ hiện khi là slide hiện tại và không đang transition */}
                      {index === currentIndex && !isTransitioning && (
                        <div
                          className={`h-full bg-[#46ca43] rounded-full transition-all ${
                            isSlideEffectActive ? "slide-effect" : ""
                          }`}
                          style={{
